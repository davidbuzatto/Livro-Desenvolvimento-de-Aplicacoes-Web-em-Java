...
Ao fazer isso, o diálogo de adição de bibliotecas será exibido (Figura~\ref{fig:cap03ConfigurarJSTLP02}). Nesse diálogo serão mostradas as bibliotecas do projeto. Como nossos projetos são configurados para conter uma cópia das bibliotecas que utilizaremos, aparecerá apenas um item, que é uma biblioteca que o NetBeans usa para copiar as bibliotecas. Vamos clicar no botão \destaque{\textit{Import...}} para importar a biblioteca da JSTL que o NetBeans oferece para nós.
...
Vamos selecionar a última versão da JSTL que estiver disponível. Na minha versão do NetBeans é a 3.0.2 (Figura~\ref{fig:cap03ConfigurarJSTLP04}). Depois de selecionar, clique em \destaque{\textit{Import Library}}.
...
Com isso, você notará que o nó \textit{Libraries} do projeto conterá mais dois itens que representam a JSTL. Agora vamos voltar ao arquivo JSP que criamos anteriormente. Vamos editá-lo? Copie então o código da Listagem~\thechapter.\ref{listagem:projetos/capitulo03/UsandoELeTagLibs/web/testesJSTL.jsp}.
