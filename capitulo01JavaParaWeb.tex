...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{protocolo:} É a parte da URL que diz ao servidor qual o protocolo a ser utilizado. Quando acessamos páginas Web, o protocolo utilizado é o \textit{Hypertext Transfer Protocol} (HTTP) ou a sua versão segura, o \textit{Hypertext Transfer Protocol Secure} (HTTPS). A diferença principal entre os dois é que o HTTPS criptografa os dados trafegados entre o cliente e o servidor, garantindo que ninguém consiga interceptá-los pelo caminho. Hoje em dia a grande maioria dos sites utiliza HTTPS, e os navegadores costumam exibir um aviso caso você acesse um site que ainda usa apenas HTTP;
...
Vamos fazer mais um teste? Preencha novamente a barra de endereços no seu navegador com o endereço \texttt{https://avatars.githubusercontent.com/u/313050} e tecle \texttt{<ENTER>}. Deve ter aparecido a minha foto, certo? Vamos analisar a URL: usamos o protocolo HTTPS, para pedir para o Servidor Web que está executando na máquina \texttt{avatars.githubusercontent.com} o recurso \texttt{313050}, que está armazenado no caminho \texttt{/u/}. Este processo é ilustrado na Figura~\ref{fig:cap01ExemploRequestResponse}.
...
Da mesma forma que existem navegadores e Servidores Web diferentes, adivinhe só, existem também Servidores de Aplicações e Contêineres de Servlets diferentes! Iremos utilizar a implementação de referência das especificações do Jakarta EE 11, que é feita pelo Contêiner de Servlets Apache Tomcat 11. Nesta edição do livro utilizaremos a versão 11.0.22, mas pode ser que na época que você estiver seguindo este material, novas versões menores da 11 possam estar disponíveis, como por exemplo 11.0.23, sendo assim, desde que você se mantenha na versão 11, não haverá nenhum problema. Trabalharemos especificamente com essa versão, pois será a ideal para o que precisaremos fazer e que tem um melhor suporte no NetBeans. Quando estamos no mundo ``Java para Web'', várias dúvidas surgem o tempo todo, visto que existe uma infinidade de termos diferentes e que normalmente causam confusão, além de existir um ecossistema absurdamente vasto. Na próxima seção vamos aprender mais alguns detalhes teóricos e na última seção vamos realizar algumas atividades!
...
Tudo evolui. O que não evolui é descartado e/ou substituído. A primeira especificação dos Servlets foi lançada em 1997 e de lá para cá, a especificação foi evoluindo, permitindo que os Servlets se tornassem componentes mais versáteis e mais fáceis de utilizar. Na versão 11.0.22 do Apache Tomcat, que implementa as especificações do Jakarta EE 11, é implementada a especificação 6.1 dos Servlets, a especificação 6.1 das JSPs (JavaServer Pages), entre outras.
...
A rigor, um JSP é processado pelo Contêiner de Servlets e todo o seu conteúdo é traduzido em um Servlet, que por sua vez é compilado e executado pelo Contêiner. Todo esse processo é realizado nos bastidores, então não precisamos nos preocupar com esses detalhes, mas é sempre bom saber um pouquinho como as coisas funcionam não é mesmo?
...
Nesta seção você aprenderá a instalar e configurar a IDE Apache NetBeans e o Contêiner de Servlets Apache Tomcat 11.0.22.


\subsection{Apache NetBeans}

Você conhece a linguagem Java, aprendeu a teoria e a prática da programação orientada a objetos e provavelmente usou a IDE Apache NetBeans para escrever seus códigos. Neste livro trabalharemos com a versão 30 desta IDE e nos passos abaixo é explicado como deve ser feito o \textit{download} e a instalação da mesma. Esses passos podem ser pulados caso você já tenha essa versão da ferramenta instalada em seu computador. Além disso, na playlist disponível no \textit{link} \url{https://www.youtube.com/playlist?list=PLqEuQ0dDknqVcfcBHGaYrET7IBfchVS-U} você encontrará tutoriais de preparação de ambientes de desenvolvimento para trabalhar com Java para Web, que serão mantidos atualizados.

\begin{enumerate}

    \item Acesse o endereço: \url{https://github.com/codelerity/netbeans-packages/releases/download/v30-build1/Apache-NetBeans-30.exe};
    
    \item Ao acessar esse \textit{link}, a versão 30 do Apache NetBeans será baixada;
    
    \item Baixou? Execute o instalador. As versões atuais dos instaladores NetBeans farão a instalação completa da IDE, bastando seguir o assistente. Essa versão em questão trará uma versão do JDK consigo, no caso, a versão 26. Aguarde o fim da instalação e vamos continuar.

\end{enumerate}

\begin{observacao}
    O instalador do NetBeans 30 já traz embutido o JDK na versão 26, então você não precisa instalá-lo separadamente. Neste livro, porém, utilizaremos o Java na versão 25. Acontece que a versão 25 é uma versão LTS (\textit{Long-Term Support}, ou suporte de longo prazo), ou seja, uma versão que recebe atualizações e correções de segurança por um período estendido (vários anos), sendo a escolha recomendada para projetos que buscam estabilidade. Já a versão 26 é uma versão intermediária, com suporte de curta duração. Na prática, não há nenhuma diferença significativa entre as duas para o conteúdo que desenvolveremos, então, se preferir, fique à vontade para usar a 26 que acompanha o instalador. Caso queira garantir o uso da versão 25, basta baixá-la separadamente e configurá-la como plataforma Java padrão no NetBeans.
\end{observacao}

Com o NetBeans instalado, abra-o. Caso o esteja executando pela primeira vez, uma página de boas-vindas será exibida. Você pode fechá-la se quiser, além de marcar para que não seja exibida novamente. Antes de criarmos o nosso primeiro projeto Java Web, precisamos baixar, instalar e configurar o Contêiner de Servlets Apache Tomcat.


\subsection{Apache Tomcat}

Como já dito, iremos instalar a versão 11.0.22 do Apache Tomcat. Para isso, primeiramente precisamos baixá-lo. Comece acessando o endereço \url{https://tomcat.apache.org/}. Do lado esquerdo, no menu, na seção \textit{Download}, escolha a versão 11 (Tomcat 11 - \url{https://tomcat.apache.org/download-11.cgi}). Logo abaixo, em \textit{Binary Distributions}, baixe a versão \textbf{Windows zip} se estiver usando Windows (seta vermelha na Figura~\ref{fig:cap01DownloadTomcat}) ou somente \textbf{zip} se estiver usando Linux ou outro sistema operacional baseado em UNIX (seta verde na Figura~\ref{fig:cap01DownloadTomcat}).

\FloatBarrier
\begin{figure}[!htbp]
    \centering
    \caption{Opções de download do Apache Tomcat}
    \includegraphics[scale=0.7]{imagens/cap01DownloadTomcat}
    \\\textbf{Fonte:} Elaborada pelo autor
    \label{fig:cap01DownloadTomcat}
\end{figure}
\FloatBarrier

Escolha onde quer salvar o arquivo e descompacte-o. No meu caso, como estou usando Windows, baixei o arquivo, salvei na minha pasta de usuário e o descompactei, ficando com o caminho \texttt{C:\textbackslash Users\textbackslash David\textbackslash apache-tomcat-11.0.22}. Veja na Figura~\ref{fig:cap01TomcatDescompactado}.

\FloatBarrier
\begin{figure}[!htbp]
    \centering
    \caption{Apache Tomcat descompactado}
    \includegraphics[scale=0.7]{imagens/cap01TomcatDescompactado}
    \\\textbf{Fonte:} Elaborada pelo autor
    \label{fig:cap01TomcatDescompactado}
\end{figure}
\FloatBarrier

Agora que estamos com o Tomcat baixado e descompactado, vamos registrá-lo no NetBeans. Com o NetBeans aberto, do lado esquerdo, clique na aba \textit{Services}. Nessa aba, há um nó chamado \destaque{\textit{Servers}}. Clique com o botão direito do mouse nele e escolha \destaque{\textit{Add Server...}}. Veja a Figura~\ref{fig:cap01Servers}.
...
Ao clicar em \destaque{\textit{Add Server...}} o assistente para registrar o servidor no NetBeans aparecerá. No primeiro passo, mostrado na Figura~\ref{fig:cap01AddServerP01}, na lista de servidores suportados, escolha \destaque{\textit{Apache Tomcat or TomEE}}. Na caixa de texto \destaque{\textit{Name:}} edite o nome da instância do servidor. No meu caso, adicionei a versão do mesmo, ficando ``Apache Tomcat 11.0.22''. Ao fazer isso, clique em \destaque{\textit{Next >}}.
...
No segundo passo do assistente precisamos definir onde os arquivos do Tomcat estão, como apresentado na Figura~\ref{fig:cap01AddServerP02}. Nesse passo, clique no botão \destaque{\textit{Browse...}} e defina onde a instalação está. No meu caso, escolhi a pasta \texttt{apache-tomcat-11.0.22} dentro da pasta do meu usuário, como já expliquei anteriormente. Recomendo que você faça o mesmo. Preencha o campo \destaque{\textit{Username}} com \texttt{admin} e o campo \destaque{\textit{Password}} com \texttt{admin} também, além de marcar a opção \destaque{\textit{Create user if it does not exist}}. Isso fará com que o NetBeans configure o usuário administrador do Tomcat. Nós não precisaremos disso durante o livro, mas é bom deixar resolvido. Você pode também definir o usuário e senha que quiser, mas recomendo seguir o que eu fiz para não ter problemas depois de esquecer esses dados, caso seja necessário. Clique em \destaque{\textit{Finish}}.
...
Ao fazer isso, o Tomcat estará pronto para ser utilizado. Caso haja alguma dúvida, visite o \textit{link} mencionado que contém uma playlist de tutoriais para a configuração de ambientes de desenvolvimento.
...
\begin{itemize}
    ...
    \item \textbf{Passo 3:} Na opção \destaque{\textit{Server:}} escolha o ``Apache Tomcat 11.0.22'', ou a opção com o nome que você definiu ao registrar o Tomcat. Em \destaque{\textit{Java EE Version:}} escolha ``Jakarta EE 11 Web''. Em \destaque{\textit{Context Path:}} deixe o valor padrão (/OlaMundoWeb), que é o mesmo nome que demos ao nosso projeto. Clique em \destaque{\textit{Next >}};
...
\begin{saibaMais}
    Existem diversas definições para \textit{framework}, sendo que, informalmente, podemos defini-los como um conjunto de classes que incorporam uma abstração que tem como objetivo de solucionar problemas de um tipo ou domínio específico.
\end{saibaMais}

Muito bem, criamos nosso primeiro projeto. Vamos executá-lo para ver o que acontece? Na barra de ferramentas do NetBeans tem um botão com uma seta verde, igual a um botão de ``\textit{play}'' de um reprodutor de mídias. Quando você clicar nesse botão, você vai ver que várias mensagens começarão a aparecer na janela de saída do NetBeans. Essas mensagens irão mostrar para nós o que está acontecendo no momento, como a inicialização do Tomcat (caso não esteja iniciado) etc. O que está esperando? Clique lá no ``\textit{play}''. Assim que tudo estiver pronto, será aberta uma janela do seu navegador, onde será mostrado o conteúdo do \texttt{index.html}, que no nosso caso será uma página com ``TODO write content'' escrito.
...
\begin{itemize}
    ...
    \item \textbf{Passo 4:} Para que uma classe seja um Servlet, precisamos estender a classe \texttt{HttpServlet}, que está contida no pacote \texttt{jakarta.servlet.http}\footnote{O Jakarta EE 11 usa o novo \textit{namespace} do Java EE, onde o pacote base é o \texttt{jakarta}. Antes da versão 9 o namespace base era \texttt{javax}.} e então implementar os métodos HTTP que queremos que nosso Servlet trate. Não se preocupe, ainda vamos aprender sobre os métodos HTTP, então o mais importante a saber, por enquanto, é que os métodos HTTP mais usados são o \texttt{GET} (\inlineJavaCode{doGet(...)} de \texttt{HttpServlet}) e o  \texttt{POST} (\inlineJavaCode{doPost(...)} de  \texttt{HttpServlet}). Então teremos que sobrescrever cada um desses métodos e ainda criaremos um terceiro que será invocado a partir dos outros dois. Confuso? Vamos ver como o código ficaria. Leia os comentários e copie o código da Listagem~\thechapter.\ref{listagem:projetos/capitulo01/OlaMundoWeb/src/java/olamundoweb/servlets/OlaServlet.java} para o seu editor.
