...
Nos exemplos 10 e 11 trataremos das requisições assíncronas e de intercâmbio de dados entre cliente e servidor.

No exemplo 12 trataremos do uso da \textit{tag} \inlineHTMLCode{<canvas>} usada para desenhar programaticamente, realizando uma simulação física de bolinhas.
...
A ideia central presente neste capítulo é fazer com que você entenda um pouco sobre JavaScript para podermos ter formulários mais sofisticados, permitindo a construção de cadastros que envolvam relacionamentos muitos-para-muitos. Claro que estamos vendo várias coisas a mais, mas a ideia é dar subsídios para implementarmos coisas novas no projeto iniciado no capítulo~\ref{cap:sistemaControleClientes}. No exemplo apresentado na Listagem~\thechapter.\ref{listagem:projetos/capitulo07/ExemplosEmJavaScript/web/js/exemplo08.js} veremos a obtenção e a inserção de dados em componentes de formulários. Detalharei alguns pontos do código e, para não ficar muito maçante, o restante é com você. Tenho certeza que entenderá o que está acontecendo com base no que foi visto até agora.
...
\begin{saibaMais}
    Mais sobre a API Web Workers: \url{https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API}.
\end{saibaMais}
...
Este exemplo é parecido com o anterior, com a diferença que agora o servidor, ao invés de responder texto puro, nos enviará dados em JSON. Sim, JSON é texto puro também, mas como o servidor informará que o texto que está vindo é em JSON, as funções conseguirão fazer a desserialização dos dados de forma automática para que possamos tratá-los por causa do cabeçalho da resposta. Na Listagem~\thechapter.\ref{listagem:projetos/capitulo07/ExemplosEmJavaScript/web/js/exemplo11.js} podem ser vistas as definições das duas funções, novamente uma usando jQuery e a outra a Fetch API.
...
Nesta Seção falaremos um pouco sobre o \textit{tag} \inlineHTMLCode{<canvas>} e sobre a Canvas API, que nos permitirá realizar operações de desenho em Duas Dimensões (2D). Atualmente, além de trabalhar com desenhos em duas dimensões, podemos também usar a WebGL API para realizar desenhos em 2D ou Três Dimensões (3D) usando aceleração em \textit{hardware}, mas isso foge demais do escopo deste livro. Focaremos no 2D realizando a simulação da física de bolinhas que serão animadas para que você possa conhecer um pouco sobre a Canvas API e sobre a utilização da função \inlineJavaScriptCode{requestAnimationFrame(...)} que nos permitirá executar código de animação de forma eficiente e sincronizada com a taxa de atualização do monitor. Na Listagem~\thechapter.\ref{listagem:projetos/capitulo07/ExemplosEmJavaScript/web/js/exemplo12.js} podemos ver o código completo do exemplo.

\javaScriptCode{Exemplo 12\newline%
Arquivo: \texttt{/js/exemplo12.js}}{projetos/capitulo07/ExemplosEmJavaScript/web/js/exemplo12.js}

Note que esse exemplo é carregado como um módulo JavaScript, como pode ser visto no \texttt{index.jsp} onde a \textit{tag} \inlineHTMLCode{<script>} que o carrega possui o atributo \inlineHTMLCode{type="module"}. Módulos JavaScript possuem escopo próprio, o que significa que as variáveis e funções declaradas neles não ficam acessíveis no escopo global. Além disso, o código de um módulo é executado automaticamente quando carregado, o que nos permite invocar a função \inlineJavaScriptCode{iniciar()} na linha 270, sem precisar chamá-la a partir do HTML.

Nas linhas 3 a 16 são declaradas as variáveis globais do módulo. A constante \inlineJavaScriptCode{GRAVIDADE} define o valor da aceleração gravitacional usada na simulação. As variáveis \inlineJavaScriptCode{ultimoTempo} e \inlineJavaScriptCode{delta} são usadas para calcular a variação de tempo entre dois quadros consecutivos da animação, o que permite que a simulação seja independente da taxa de quadros por segundo (\textit{frames per second}, ou FPS) do dispositivo. As demais variáveis armazenam o contexto gráfico, o array de bolinhas, a bolinha selecionada pelo mouse, as dimensões do \textit{canvas} e o estado do mouse.

Entre as linhas 19 e 113 é definida a classe \texttt{Bolinha}. O construtor, na linha 21, recebe seis parâmetros:

\begin{itemize}
    \item \inlineJavaScriptCode{x}: posição do centro da bolinha no eixo $x$;
    \item \inlineJavaScriptCode{y}: posição do centro da bolinha no eixo $y$;
    \item \inlineJavaScriptCode{raio}: raio do círculo que representa a bolinha;
    \item \inlineJavaScriptCode{vx}: velocidade inicial no eixo $x$;
    \item \inlineJavaScriptCode{vy}: velocidade inicial no eixo $y$;
    \item \inlineJavaScriptCode{cor}: cor usada para desenhar a bolinha.
\end{itemize}

Além desses, o construtor inicializa os atributos \inlineJavaScriptCode{atrito} e \inlineJavaScriptCode{elasticidade}, que simulam as forças físicas aplicadas à bolinha nas colisões e no movimento, e \inlineJavaScriptCode{emArraste}, que indica se a bolinha está sendo arrastada com o mouse.

O método \inlineJavaScriptCode{atualizar( delta )} (linha 49) é responsável por atualizar o estado da bolinha a cada quadro da animação. O parâmetro \inlineJavaScriptCode{delta} é a variação de tempo em segundos desde o último quadro. Usando esse valor, a posição da bolinha é atualizada nas linhas 55 e 56 multiplicando a velocidade pelo \inlineJavaScriptCode{delta}, garantindo que o movimento seja independente da taxa de quadros. Em seguida, entre as linhas 59 e 74, são feitas as verificações de colisão com as bordas do \inlineHTMLCode{<canvas>}: se a bolinha ultrapassar alguma borda, ela é reposicionada e sua velocidade é invertida e multiplicada pela elasticidade, simulando a perda de energia na colisão. Nas linhas 77 e 78, as velocidades são recalculadas aplicando o atrito e a gravidade, sendo que a gravidade também é multiplicada por \inlineJavaScriptCode{delta} para manter a simulação independente da taxa de quadros. Caso a bolinha esteja sendo arrastada pelo mouse (bloco \inlineJavaScriptCode{else} a partir da linha 80), sua posição é ajustada para seguir o cursor e sua velocidade é calculada com base no deslocamento entre quadros, para que ao ser solta ela continue se movendo de forma natural.

O método \inlineJavaScriptCode{desenhar( ctx )} (linha 97) realiza o desenho da bolinha no contexto gráfico. O método salva o estado do contexto na linha 98, define a cor de preenchimento, inicia um caminho, desenha um arco completo na linha 101 --- que resulta no círculo da bolinha --- e preenche o caminho com a cor definida. Na linha 103 o estado salvo é restaurado, garantindo que as configurações do contexto não se propaguem para os outros elementos desenhados.

O método \inlineJavaScriptCode{intercepta( x, y )} (linha 107) retorna \inlineJavaScriptCode{true} se a coordenada fornecida estiver dentro do círculo da bolinha. O cálculo é feito usando o teorema de Pitágoras de forma otimizada: em vez de calcular a raiz quadrada da distância, compara-se o quadrado da distância com o quadrado do raio.

A função \inlineJavaScriptCode{iniciar()} (linha 116) é chamada automaticamente ao carregar o módulo. Ela obtém o \inlineHTMLCode{<canvas>} e seu contexto gráfico 2D, armazena as dimensões do \textit{canvas}, cria a primeira bolinha e registra os ouvintes de eventos do mouse. São tratados quatro tipos de evento: ao pressionar o botão esquerdo do mouse sobre uma bolinha ela passa a ser arrastada; ao pressionar o botão do meio, todas as bolinhas recebem velocidades aleatórias; ao pressionar e arrastar com o botão direito, novas bolinhas são criadas; e ao mover o mouse para fora do \textit{canvas}, o arraste é cancelado. Por fim, a animação é iniciada com a chamada \inlineJavaScriptCode{requestAnimationFrame( executar )} na linha 202.

A função \inlineJavaScriptCode{executar( tempoAtual )} (linha 207) é o coração do \textit{loop} de animação. A cada quadro, ela calcula o \inlineJavaScriptCode{delta} em segundos (linha 211), atualiza e desenha a simulação e então agenda o próximo quadro com outra chamada a \inlineJavaScriptCode{requestAnimationFrame}. Diferentemente de \inlineJavaScriptCode{setInterval}, a função \inlineJavaScriptCode{requestAnimationFrame} é sincronizada com a taxa de atualização do monitor, resultando em animações mais suaves e eficientes, além de pausar automaticamente quando a aba do navegador não estiver visível.
...
\section{Recursos Modernos do JavaScript}

Nos exemplos anteriores já utilizamos algumas construções do JavaScript moderno --- em especial nas funções de \textit{callback} e nos exemplos assíncronos --- sem formalizá-las. Esta seção preenche essa lacuna, apresentando de forma concisa os recursos da linguagem que você encontrará nos projetos dos capítulos seguintes. Nenhum novo arquivo de projeto será criado: os exemplos são apresentados diretamente no texto para você experimentar no console do navegador (a tecla \texttt{F12} em qualquer navegador moderno abre as ferramentas do desenvolvedor) ou em qualquer editor \textit{online} como o JSFiddle\footnote{\url{https://jsfiddle.net/}}.


\subsection{\textit{Arrow Functions}}

Uma \destaque{\textit{arrow function}} (função de seta) é uma sintaxe compacta para declarar funções anônimas, introduzida no ES2015. Além da sintaxe menor, ela tem uma diferença semântica importante: não cria seu próprio \inlineJavaScriptCode{this}, herdando o \inlineJavaScriptCode{this} do escopo em que foi definida. Isso a torna a escolha natural para funções de \textit{callback}.

\begin{boxJavaScriptCode}[title=Arrow functions]
// função anônima tradicional
const dobrarTradicional = function( x ) {
    return x * 2;
};

// arrow function equivalente - retorno implícito quando o corpo
// é uma única expressão (sem chaves e sem return)
const dobrar = ( x ) => x * 2;

// os parênteses são opcionais quando há apenas um parâmetro
const dobrarSemParenteses = x => x * 2;

// com múltiplos parâmetros os parênteses são obrigatórios
const somar = ( a, b ) => a + b;

// com corpo de múltiplas linhas as chaves e o return são obrigatórios
const processar = ( x ) => {
    const resultado = x * 2;
    return resultado + 1;
};

// uso típico como callback - muito mais conciso do que function( n ) {...}
const numeros = [1, 2, 3, 4];
numeros.forEach( n => console.log( n ) ); // imprime 1, 2, 3, 4
\end{boxJavaScriptCode}

Você já viu \textit{arrow functions} nos exemplos anteriores, passadas como \textit{callbacks} para o \inlineJavaScriptCode{forEach}. Nos capítulos seguintes elas aparecem em praticamente todos os módulos JavaScript dos projetos.

\begin{saibaMais}
    Documentação sobre \textit{arrow functions} na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions}.
\end{saibaMais}


\subsection{Parâmetros com Valor Padrão}

É possível definir um valor padrão para um parâmetro de função. Quando o argumento correspondente não for fornecido --- ou for \inlineJavaScriptCode{undefined} --- o valor padrão é usado automaticamente.

\begin{boxJavaScriptCode}[title=Parâmetros com valor padrão]
// o parâmetro "saudacao" tem o valor padrão "Olá"
function cumprimentar( nome, saudacao = "Olá" ) {
    return `${saudacao}, ${nome}!`;
}

console.log( cumprimentar( "Bia" ) );           // "Olá, Bia!"
console.log( cumprimentar( "Bia", "Oi" ) );     // "Oi, Bia!"

// combinado com arrow function
...
No capítulo~\ref{cap:webservicesRestful}, a função \inlineJavaScriptCode{customFetch} usa esse recurso em sua assinatura:

\begin{boxJavaScriptCode}
customFetch( url, metodo, dados = null, propToken = "token" )
\end{boxJavaScriptCode}

tornando os dois últimos argumentos opcionais.

\begin{saibaMais}
    Documentação sobre parâmetros com valor padrão na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Default_parameters}.
\end{saibaMais}


\subsection{Funções de Ordem Superior: \texttt{map} e \texttt{filter}}

Além do \inlineJavaScriptCode{forEach}, \inlineJavaScriptCode{every} e \inlineJavaScriptCode{some} que já vimos, o tipo Array possui outros dois métodos de ordem superior muito úteis: \inlineJavaScriptCode{map} e \inlineJavaScriptCode{filter}. Enquanto o \inlineJavaScriptCode{forEach} itera pelo array sem retornar nada, o \inlineJavaScriptCode{map} retorna um novo array com o resultado da transformação de cada elemento, e o \inlineJavaScriptCode{filter} retorna um novo array contendo apenas os elementos que satisfazem uma condição.

\begin{boxJavaScriptCode}[title=map e filter]
...
// map e filter podem ser encadeados
const dobradosDosImpares = numeros
    .filter( n => n % 2 !== 0 )   // [1, 3, 5]
    .map( n => n * 2 );           // [2, 6, 10]

// exemplo com array de objetos - muito comum ao processar dados de APIs
const pessoas = [
    { nome: "Ana",   idade: 30 },
    { nome: "Bruno", idade: 17 },
    { nome: "Carla", idade: 25 }
...
const nomesAdultos = pessoas
    .filter( p => p.idade >= 18 )
    .map( p => p.nome );

console.log( nomesAdultos ); // ["Ana", "Carla"]
\end{boxJavaScriptCode}

\begin{saibaMais}
    Documentação sobre \inlineJavaScriptCode{map} na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map}.\\
    Documentação sobre \inlineJavaScriptCode{filter} na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter}.
\end{saibaMais}


\subsection{Desestruturação}

A \destaque{desestruturação} (\textit{destructuring}) é uma sintaxe que permite extrair valores de arrays ou propriedades de objetos para variáveis distintas de forma concisa.

\begin{boxJavaScriptCode}[title=Desestruturação de objetos e arrays]
// --- desestruturação de objetos ---
const produto = { id: 1, descricao: "Caneta", preco: 2.50 };

// sem desestruturação
const idManual = produto.id;
const descricaoManual = produto.descricao;

// com desestruturação (equivalente, mais conciso)
const { id, descricao, preco } = produto;
console.log( descricao ); // "Caneta"

// com renomeação da variável extraída
const { descricao: nomeProduto } = produto;
console.log( nomeProduto ); // "Caneta"

// com valor padrão para propriedades ausentes
...
function exibir( { descricao, preco } ) {
    console.log( `${descricao}: R$ ${preco.toFixed(2)}` );
}
exibir( produto ); // "Caneta: R$ 2.50"
\end{boxJavaScriptCode}

\begin{saibaMais}
    Documentação sobre desestruturação na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment}.
\end{saibaMais}


\subsection{Encadeamento Opcional e Coalescência Nula}

Dois operadores modernos muito úteis ao lidar com dados vindos de APIs são o \destaque{encadeamento opcional} (\inlineJavaScriptCode{?.}) e a \destaque{coalescência nula} (\inlineJavaScriptCode{??}).

O operador \inlineJavaScriptCode{?.} permite acessar propriedades aninhadas de um objeto sem precisar verificar manualmente se cada nível é nulo. Se qualquer parte da cadeia for \inlineJavaScriptCode{null} ou \inlineJavaScriptCode{undefined}, a expressão inteira retorna \inlineJavaScriptCode{undefined} em vez de lançar um \texttt{TypeError}.

O operador \inlineJavaScriptCode{??} retorna o operando da direita apenas quando o da esquerda é \inlineJavaScriptCode{null} ou \inlineJavaScriptCode{undefined}. Ele é diferente do operador lógico \inlineJavaScriptCode{||}, que retorna o operando da direita para qualquer valor \textit{falsy} (como \inlineJavaScriptCode{0}, \inlineJavaScriptCode{""} ou \inlineJavaScriptCode{false}).

\begin{boxJavaScriptCode}[title=Encadeamento opcional e coalescência nula]
const usuario = {
    nome: "Ana",
    endereco: { cidade: "São Carlos" }
};

// sem ?. - lança TypeError se "endereco" for null ou undefined
const cidadeDireta = usuario.endereco.cidade;

// com ?. - retorna undefined sem lançar erro
const cidade = usuario?.endereco?.cidade;
console.log( cidade ); // "São Carlos"

const cep = usuario?.endereco?.cep;
console.log( cep ); // undefined (sem erro)

// também funciona em chamadas de métodos
const maiusculo = usuario?.nome?.toUpperCase();
console.log( maiusculo ); // "ANA"
...
const estoque = 0;
console.log( estoque || 10 ); // 10 -- indesejado! 0 é falsy, foi substituído
console.log( estoque ?? 10 ); // 0  -- correto: ?? só substitui null/undefined
\end{boxJavaScriptCode}

\begin{saibaMais}
    Documentação sobre o operador \inlineJavaScriptCode{?.} na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Optional_chaining}.\\
    Documentação sobre o operador \inlineJavaScriptCode{??} na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Nullish_coalescing}.
\end{saibaMais}


\subsection{Programação Assíncrona com \texttt{async} e \texttt{await}}

No exemplo 10 deste capítulo usamos a Fetch API com \inlineJavaScriptCode{.then()} e \inlineJavaScriptCode{.catch()} para lidar com o resultado assíncrono de uma requisição. Essa abordagem funciona bem, mas pode se tornar difícil de ler quando há múltiplas operações assíncronas dependentes uma da outra.

As palavras-chave \inlineJavaScriptCode{async} e \inlineJavaScriptCode{await}, introduzidas no ES2017, permitem escrever código assíncrono com a mesma estrutura linear do código síncrono, sem abrir mão das \textit{Promises} por baixo dos panos. Uma função marcada com \inlineJavaScriptCode{async} sempre retorna uma \textit{Promise}, e dentro dela podemos usar \inlineJavaScriptCode{await} para pausar a execução até que uma \textit{Promise} seja resolvida.

\begin{boxJavaScriptCode}[title=async/await vs. Promises com .then()]
// --- com .then() (Promises encadeadas) ---
function buscarProdutosComThen() {
    fetch( "/api/produtos" )
...
// --- com async/await (equivalente, estrutura mais linear e legível) ---
...
// a vantagem fica ainda mais clara com múltiplas operações dependentes
async function salvarERecarregar( cidade ) {
    ...
    if ( response.ok ) {
        await carregar(); // aguarda o recarregamento antes de continuar
    }
    return resultado;
}
\end{boxJavaScriptCode}

No capítulo~\ref{cap:webservicesRestful}, todas as funções que se comunicam com o servidor (\inlineJavaScriptCode{carregar}, \inlineJavaScriptCode{salvar}, \inlineJavaScriptCode{excluir}) são declaradas com \inlineJavaScriptCode{async} e usam \inlineJavaScriptCode{await} para aguardar as respostas.

\begin{saibaMais}
    Documentação sobre \inlineJavaScriptCode{async}/\inlineJavaScriptCode{await} na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function}.
\end{saibaMais}


\subsection{Módulos ES}

Os \destaque{módulos ES} (\textit{ES Modules}) são o sistema nativo de módulos do JavaScript moderno. Eles permitem dividir o código em arquivos separados, controlando o que cada arquivo expõe e o que cada arquivo consome de outros. Isso é fundamental para a organização de aplicações maiores --- sem módulos, todas as variáveis e funções ficam no escopo global e o risco de conflitos de nome cresce rapidamente.

Para carregar um arquivo JavaScript como módulo no HTML, basta adicionar o atributo \inlineHTMLCode{type="module"} à \textit{tag} \inlineHTMLCode{<script>}:

\begin{boxHTMLCode}
<script type="module" src="/src/js/app.js"></script>
\end{boxHTMLCode}

Dentro dos arquivos JavaScript, usamos \inlineJavaScriptCode{export} para expor o que outros arquivos podem usar, e \inlineJavaScriptCode{import} para consumir o que outros arquivos exportam.

\begin{boxJavaScriptCode}[title=Exportando e importando com módulos ES]
// --- em utils.js: exportando ---

// named exports: pode haver vários por arquivo
...
// --- em app.js: importando ---

// importando itens específicos pelo nome (named imports)
import { somar, PI } from "./utils.js";
console.log( somar( 2, 3 ) ); // 5
console.log( PI );            // 3.14159

// importando tudo com um alias (namespace import)
import * as Utils from "./utils.js";
console.log( Utils.somar( 2, 3 ) ); // 5


// --- módulos têm escopo próprio ---
// variáveis declaradas aqui NÃO são globais
let contador = 0; // visível apenas dentro deste arquivo
\end{boxJavaScriptCode}

\begin{atencao}
    Módulos ES só funcionam quando o arquivo HTML é servido por um servidor HTTP --- não funcionam ao abrir o arquivo diretamente no navegador via protocolo \texttt{file://}. Nos projetos deste livro isso não é problema, pois o Apache Tomcat e o \textit{Live Server} fazem esse papel.
\end{atencao}

No capítulo~\ref{cap:webservicesRestful}, toda a arquitetura do \textit{front-end} é construída com módulos ES: cada cadastro (\texttt{estados.js}, \texttt{cidades.js}, \texttt{produtos.js} etc.) é um módulo independente que exporta apenas a função \inlineJavaScriptCode{iniciar}, e o \texttt{app.js} importa e orquestra todos eles.

\begin{saibaMais}
    Documentação sobre módulos ES na MDN: \url{https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules}.
\end{saibaMais}
...
Chegamos ao fim de mais um capítulo. Vimos as construções fundamentais do JavaScript --- variáveis, condicionais, laços, arrays, objetos e JSON ---, a manipulação do DOM, o tratamento de eventos, requisições assíncronas com a Fetch API e jQuery, e finalizamos com uma simulação física usando a Canvas API. Na seção de recursos modernos formalizamos as construções que você encontrará nos projetos dos capítulos seguintes: \textit{arrow functions}, parâmetros com valor padrão, \inlineJavaScriptCode{map} e \inlineJavaScriptCode{filter}, desestruturação, encadeamento opcional (\inlineJavaScriptCode{?.}), coalescência nula (\inlineJavaScriptCode{??}), programação assíncrona com \inlineJavaScriptCode{async}/\inlineJavaScriptCode{await} e módulos ES. É importante que você tenha um bom domínio sobre esses recursos e que, após a leitura deste capítulo, seja capaz de continuar seu aprendizado de forma independente. No próximo capítulo usaremos JavaScript para implementar um formulário de venda de produtos e AJAX para o cancelamento de vendas.
...
\begin{projetoSemArquivo}{}{}{}
    Implemente um cadastro simples (apenas a inserção) de dados de uma ``fruta''. A tabela deve ter como colunas um campo identificador (\inlineSQLCode{INT}), um campo que armazenará o nome da fruta (\inlineSQLCode{VARCHAR}) e um campo para armazenar a cor predominante da fruta (\inlineSQLCode{VARCHAR}). Deve-se listar as frutas cadastradas usando AJAX, montando uma tabela dinamicamente, ao invés de processar os objetos no JSP do lado do servidor. Note que esse projeto é parecido com o Projeto~\ref{cap:padroesDeProjeto}.1 do capítulo~\ref{cap:padroesDeProjeto}.
