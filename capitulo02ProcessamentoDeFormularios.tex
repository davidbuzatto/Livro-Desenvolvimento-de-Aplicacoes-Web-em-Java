...
\begin{atencao}
    De acordo com a especificação do protocolo HTTP (RFC 9110), os nomes dos métodos são \textbf{sensíveis a maiúsculas e minúsculas} (\textit{case-sensitive}) e devem ser escritos \textbf{sempre em maiúsculas}: \texttt{GET}, \texttt{POST}, \texttt{PUT}, \texttt{DELETE} etc. Escrever \texttt{get} ou \texttt{Get}, por exemplo, é tecnicamente inválido. Alguns servidores são permissivos e aceitam qualquer caixa, mas esse comportamento não é garantido pela especificação e pode variar de servidor para servidor. Para evitar problemas de compatibilidade, use sempre letras maiúsculas ao referenciar métodos HTTP.
\end{atencao}
...
Parece grego, mas não é! Vamos entender a URL. Estamos usando o protocolo HTTPS, para acessar a máquina \texttt{www.google.com} e o recurso inicia em \texttt{search} e, a partir do ponto de interrogação, são codificados os parâmetros enviados na requisição do recurso:
...
\begin{observacao}
    Usar o método \texttt{POST} esconde os dados da URL, mas isso não significa que eles estejam protegidos! Se a aplicação usar apenas HTTP, os dados do corpo da requisição ainda trafegam em texto puro pela rede e podem ser interceptados. Quem garante que os dados --- tanto da URL quanto do corpo da requisição --- viajem de forma segura e criptografada é o HTTPS, como vimos no capítulo~\ref{cap:javaParaWeb}. Portanto, em aplicações reais que lidam com informações sensíveis, como senhas e dados pessoais, o uso de HTTPS é indispensável, independentemente do método HTTP utilizado.
\end{observacao}
